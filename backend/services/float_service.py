...
def validate_token(api_token: str) -> bool:
    ...
    try:
        ...
        log.info("float_validate_token status=%s ok=%s duration_ms=%.0f", resp.status_code, ok, (time.monotonic() - started) * 1000)
        return ok
    except Exception:
...
def fetch_bill_payments(
    ...
    if created_at_lte:
        params["created_at__lte"] = created_at_lte
    return _fetch_all_pages(f"{FLOAT_BASE}/v1/payments", api_token, params)
...
def fetch_reimbursements(
    ...
    if created_at_lte:
        params["created_at__lte"] = created_at_lte
    return _fetch_all_pages(f"{FLOAT_BASE}/v1/reimbursements", api_token, params)


def fetch_users(api_token: str) -> list[dict]:
    """Fetch all users in the Float organization."""
    return _fetch_all_pages(f"{FLOAT_BASE}/v1/users", api_token, {"page_size": "1000"})


def fetch_cards(api_token: str, status: str | None = None) -> list[dict]:
    """Fetch cards, optionally filtered by status (e.g. 'ACTIVE')."""
    params: dict[str, str] = {"page_size": "1000"}
    if status:
        params["status"] = status
    return _fetch_all_pages(f"{FLOAT_BASE}/v1/cards", api_token, params)
